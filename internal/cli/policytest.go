...
func newPolicyCmd(configDir *string) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "policy",
		Short: "Author, inspect, learn, and test authority policies",
	}
	cmd.AddCommand(newPolicyTestCmd(configDir))
	cmd.AddCommand(newPolicyScaffoldCmd())
	cmd.AddCommand(newPolicyLearnCmd())
	return cmd
}

// newPolicyLearnCmd turns prior signed audit events into a reviewable policy
// draft. It deliberately never edits a Charter: learning is evidence-driven,
// but accepting a proposed authority rule must remain an explicit human act.
func newPolicyLearnCmd() *cobra.Command {
	var includeAllowed bool
	cmd := &cobra.Command{
		Use:   "learn <evidence-or-chronicle.json>",
		Short: "Suggest exact policy rules from a prior audit trail",
		Long: "Read a portable evidence package or signed Chronicle bundle and print a\n" +
			"reviewable TOML draft. Denied and approval-gated actions retain their\n" +
			"verdict; --include-allowed also proposes exact rules for observed allows.\n" +
			"No policy file is modified automatically.",
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			events, err := readLearningEvents(args[0])
			if err != nil {
				return err
			}
			fmt.Fprint(cmd.OutOrStdout(), renderLearnedPolicy(events, includeAllowed))
			return nil
		},
	}
	cmd.Flags().BoolVar(&includeAllowed, "include-allowed", false, "also propose exact allow rules observed in the audit trail")
	return cmd
}
...
func readLearningEvents(path string) ([]ledger.Event, error) {
	...
	if envelope.Format != "" {
		doc, err := evidence.Read(strings.NewReader(string(b)))
		if err != nil {
			return nil, err
		}
		if _, err := evidence.Verify(doc); err != nil {
			return nil, fmt.Errorf("refusing to learn from unverified evidence: %w", err)
		}
		return doc.Chronicle.Events, nil
	}
	if len(envelope.Events) == 0 {
		return nil, fmt.Errorf("audit input contains no events")
	}
	if _, err := ledger.VerifyBundle(strings.NewReader(string(b))); err != nil {
		return nil, fmt.Errorf("refusing to learn from an unverified Chronicle bundle: %w", err)
	}
	return envelope.Events, nil
}

func renderLearnedPolicy(events []ledger.Event, includeAllowed bool) string {
	type suggestion struct{ tool, action, verdict string }
	seen := make(map[string]suggestion)
	skippedRedacted := 0
	for _, event := range events {
		verdict := event.Verdict
		if verdict == string(profile.VerdictAllow) && !includeAllowed {
			continue
		}
		if verdict != string(profile.VerdictAllow) && verdict != string(profile.VerdictDeny) && verdict != string(profile.VerdictRequireApprove) {
			continue
		}
		if event.Redacted || strings.TrimSpace(event.Action) == "" {
			skippedRedacted++
			continue
		}
		if event.Tool == "approval" || event.Tool == "fleet" {
			continue
		}
		key := event.Tool + "\x00" + event.Action + "\x00" + verdict
		seen[key] = suggestion{event.Tool, event.Action, verdict}
	}
	items := make([]suggestion, 0, len(seen))
	...
	out.WriteString("# Generated by `runeward policy learn`. Review before adding to a Charter.\n")
	out.WriteString("# Rules are intentionally exact; broaden globs only when the risk is understood.\n")
	if skippedRedacted > 0 {
		fmt.Fprintf(&out, "# Skipped %d redacted/empty action(s); plaintext is intentionally unavailable.\n", skippedRedacted)
	}
	if len(items) == 0 {
		out.WriteString("# No eligible actions found. Use --include-allowed to include observed allows.\n")
		return out.String()
	}
	for _, item := range items {
		if item.tool == "net" && validExactHostname(item.action) {
			out.WriteString("\n[[network.rule]]\n")
			fmt.Fprintf(&out, "verdict  = %s\n", strconv.Quote(item.verdict))
			fmt.Fprintf(&out, "hostname = %s\n", strconv.Quote(item.action))
			continue
		}
		out.WriteString("\n[[policy]]\n")
		fmt.Fprintf(&out, "tool    = %s\n", strconv.Quote(item.tool))
		fmt.Fprintf(&out, "match   = %s\n", strconv.Quote(item.action))
		fmt.Fprintf(&out, "verdict = %s\n", strconv.Quote(item.verdict))
		out.WriteString("reason  = \"learned from a verified audit trail; reviewed by POLICY_OWNER\"\n")
	}
	return out.String()
}

func validExactHostname(s string) bool {
...
func newPolicyTestCmd(configDir *string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short: "Simulate a policy (Charter) against sample actions",
		Long: "Load a policy (Charter) and evaluate it offline against a table of\n" +
