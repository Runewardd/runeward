<p align="center">
  <img src="docs/assets/runeward-banner.png" alt="runeward — governed execution cells for AI agents" width="820" />
</p>

<p align="center">
  <b>Governed execution cells for AI agents.</b><br/>
...
> **Status:** the full MVP surface is in place — Docker + Kubernetes backends, TOML/YAML/JSON
> profiles, a signed tamper-evident ledger, built-in/CEL/Rego policy engines + guardrails,
> deny-by-default egress, the governed REST control plane, web dashboard, MCP server, snapshots,
> multi-agent fleets, workspace seeding (`copy_from`) + `export`, and agent-native distribution.

## Why runeward

Letting an AI agent run shell commands, edit files, install packages, and hit the network is
useful right up until it `rm -rf`s the wrong directory, exfiltrates a secret, or burns your API
budget in a retry loop. Raw isolation ("jail the agent in a box") is now table stakes. **runeward
adds the governance layer _around_ the box** that most sandboxes lack.

Think of it as a **seatbelt + flight recorder** for autonomous agents:
...
- **Profiles are a security contract.** `[host]`, `[network]`, `[[env]]`, `[[file]]`, `[[policy]]`,
  ...
  default, so the blast radius is legible and intentional — not implicit.
- **Governed, not just isolated.** Every action flows through one path —
  `policy → approval gate → guardrails → backend exec → audit ledger` — no matter whether it
  arrives via REST, the dashboard, or MCP.
- **Tamper-evident by construction.** An append-only, hash-chained, **ed25519-signed** ledger
  records every call and its verdict, and exports as an independently-verifiable transcript
...
- **Cost & loop guardrails.** Hard caps on wall-clock, exec count, egress requests, and tight
  retry-loop detection stop runaway agents.
- **Pluggable backends.** A Docker/Podman backend for zero-setup laptop use, or a Kubernetes
  backend (with strict L3 egress + CRDs + admission webhook) for production and fleets. Everything
  above the backend is identical.
...
|---|---|---|
| Isolation (container/VM) | ✅ | ✅ (Docker **or** Kubernetes) |
| Deny-by-default network egress | sometimes | ✅ SNI allowlist, strict L3 on k8s |
| Per-action policy + approvals | rare | ✅ builtin / CEL / OPA-Rego + HITL gates |
| Tamper-evident, signed audit trail | rare | ✅ hash-chained + ed25519, verifiable export |
| Cost / loop guardrails | rare | ✅ wall-clock, exec, egress, loop caps |
| Multi-agent fleets | rare | ✅ N cells + atomic task board |
...
2. **By running an agent CLI _inside_ the sandbox (Codex, Cursor CLI).** A profile ships the agent
   binary in the image and injects its API key; you launch the agent with a single governed exec
   call (or a whole fleet of them). See [Running agents & fleets](#running-agents--fleets).
...
Open <http://localhost:8080> for the dashboard: pick a profile, click **New** (optionally point it
at a local folder to copy in), and drive the sandbox's terminal, files, shell, audit timeline, and
approvals inbox.

## Working against your own code (folder mounting)

runeward deliberately **never mounts your host folder** into the sandbox. Instead it takes a
**one-time copy** at create, so the agent works on an isolated `/workspace` and your real files are
never modified. There are three ways to seed it:

**1. In a profile** — set `host.copy_from`:
...
**2. Per-create over REST / dashboard** — override for a single sandbox:
...
In the dashboard, the **New** dialog has an optional *"Copy local folder into workspace"* field.

**3. Pull results back out** — copy the agent's finished workspace to a host directory (the sandbox
is only read; later host edits never flow back in):
...
`runeward_shell`, `runeward_python`, `runeward_read_file`, `runeward_browser_*`, etc. — all governed.

### Fleets (many agents, one task board)

A fleet is **N identical governed cells sharing an atomic, concurrency-safe task board**. Set the
replica count and per-agent model/prompt in the profile
([`examples/codex-fleet.toml`](examples/codex-fleet.toml), [`examples/cursor-fleet.toml`](examples/cursor-fleet.toml)):
...
Dead workers' claims auto-requeue via leases; the board survives restarts. Pin the model per agent
by baking it into the profile's launch command (e.g. `codex exec -m o3` / `agent --model ...`).

## Profiles

Profiles may be authored in **TOML, YAML, or JSON** — the file extension picks the parser, and all
three support the same schema (TOML shown throughout the docs for brevity). `runeward <name>`
resolves `<name>.{toml,yaml,yml,json}` in order:

1. `./.runeward/<name>.*` — project-local, committed with the repo.
...
sanitized templates in [`examples/`](examples/). See
[`examples/ns-auto.toml`](examples/ns-auto.toml) for a fully worked deny-by-default profile.
...
`runeward serve` routes every tool call through `policy → approval gate → guardrails → backend
exec → audit ledger`, whether it arrives over REST, the dashboard, or MCP.
...
the approvals inbox (returning `202` with an `approval_id` if it waits too long). Pin the ledger +
...
(Claude Desktop / Cursor / VS Code) or `--http` for the streamable transport (also mounted at
`/mcp` by `runeward serve`).
...
**L3** (`enforce = "strict"`: an iptables init container + transparent SNI proxy) so it can't be
...
### Org-wide policy defaults (`ClusterPolicy` + admission webhook)

A cluster-scoped `ClusterPolicy` sets org-wide guardrails on `Sandbox`/`Fleet` resources, enforced
by `runeward webhook` (a self-registering validating + mutating admission webhook that mints its own
serving cert):
...
Instead of embedding policy inline, a profile can pull it from a **signed, versioned OCI policy
bundle** so a security team ships one artifact many profiles consume:
...
The signature covers the content-addressed config + layer digests and rides in the OCI manifest
annotations; see [`examples/policy-bundle.toml`](examples/policy-bundle.toml).

## Testing

See [`docs/E2E-TESTING.md`](docs/E2E-TESTING.md) for an end-to-end local walkthrough covering the
Docker and Kubernetes backends, deny-by-default + strict egress, snapshots, multi-agent fleets, and
wiring the MCP server into Claude Desktop, Cursor, and VS Code.

## Roadmap

- [x] Profile schema + resolver (TOML/YAML/JSON) + redacted `print`
- [x] Pluggable `Backend` interface + Docker backend + interactive `enter`
- [x] Kubernetes backend (client-go Pod + PVC + exec/PTY; verified on OrbStack k8s)
- [x] `Sandbox`/`Fleet` (+ cluster-scoped) CRDs + lightweight controller (`runeward controller`)
- [x] `runeward-agent` (in-sandbox shell/code/file HTTP API, path-confined)
- [x] Signed, tamper-evident audit ledger (ed25519) + deterministic replay/export
- [x] Policy engine (builtin/CEL/Rego) + cost/loop guardrails
- [x] Control-plane REST wiring backend + ledger + policy + guardrails + approval gates (`serve`)
- [x] Interactive terminal over WebSocket + web dashboard
- [x] Deny-by-default egress allowlist proxy in both backends (+ strict L3 on k8s)
- [x] MCP server wrapping the governed tools (stdio + streamable HTTP)
- [x] Snapshots + multi-agent fleets (atomic, concurrency-safe task board)
- [x] Workspace seeding (`copy_from`) + `runeward export` for isolated copies of local code
- [x] Governed browser tool — one-shot render + stateful CDP-driven sessions, behind egress policy
- [x] `ClusterPolicy` + admission webhook (`runeward webhook`) for org-wide defaults
- [x] Signed, versioned OCI policy bundles (`[policy_bundle]` + `runeward bundle`)
- [x] Agent-native distribution — `SKILL.md`, MCP registry manifest, Python + TS adapters
- [x] Packaging — container image, Helm chart, release CI, one-command `runeward up`

Further ideas (not yet scheduled): a live browser terminal in the dashboard, OCI-distributed signed
*bundle indexes*, and cross-cluster fleet federation.
