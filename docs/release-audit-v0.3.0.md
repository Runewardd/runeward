...
| Gate | Result |
| --- | --- |
| Version consistency | Python, npm, npm lockfile, MCP implementation/manifest/OCI reference, and Helm chart/app versions agree on `0.3.0`. |
...
| npm package | `npm pack` produced `@runeward/sdk@0.3.0`; an offline clean-prefix install imported `RunewardClient`. |
| Production Charters | Every Charter in `examples/safe` passed `validate --strict`. |
| Helm | Chart lint and controller/server rendering passed with app and image tag `0.3.0`. |
