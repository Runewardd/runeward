...
| npm package | `npm pack` produced `@runeward/sdk@0.3.0-rc.2`; an offline clean-prefix install imported `RunewardClient`. |
...
| Helm | Chart lint and controller/server rendering passed with app and image tag `0.3.0-rc.2`. |
...
| Authenticated API automation | Previously manual role coverage was not a repeatable release gate. | CI now runs `scripts/e2e-rbac.sh` with six roles, tenant isolation, readiness, governed actions, capabilities, and Cohorts on Docker. |
| Dashboard browser automation | The release environment still needs a real embedded-browser run. | Verify Citadel switching clears state, Cohort resolution modal, browser text/screenshot actions, provisioning progress, and IDE-link fallback. |
| Channel installs | Homebrew, PyPI, npm, OCI, and rollback tests require published RC artifacts. | Install `v0.3.0-rc.2` on clean macOS/Linux environments and verify rollback before the stable tag. |
...
then publish `v0.3.0-rc.2` for clean-machine channel testing. Promote to
