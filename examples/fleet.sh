...
#   RUNEWARD_TOKEN=<principal token>  (required when serve uses RBAC/auth)
#
# Commands:
...
sandboxes() { api -sf "$BASE/v1/cohorts/$(fid)" | jq -r '.sandboxes[]'; }
...
run_on() { # sandbox-id, prompt -> runs agent, prints stdout, returns exit status
  api -sf "$BASE/v1/citadels/$1/shell/exec" -d "$(cmd_for "$2")" | jq -r '.stdout // .reason // ""'
}

worker() { # sandbox-id, owner: claim+build until the board is empty
  local sb="$1" owner="$2" claim tid prompt lease task_owner body
  while :; do
    claim=$(api -sf "$BASE/v1/cohorts/$(fid)/claim" -d "{\"owner\":\"$owner\"}") \
      || die "task claim failed for $owner (check RUNEWARD_TOKEN and Cohort access)"
    [ "$(echo "$claim" | jq -r '.claimed')" = "true" ] || break
    tid=$(echo "$claim" | jq -r '.task.id')
    prompt=$(echo "$claim" | jq -r '.task.payload')
    lease=$(echo "$claim" | jq -r '.task.lease_token')
    task_owner=$(echo "$claim" | jq -r '.task.owner')
    echo ">> [$owner/$sb] building: $prompt"
    if run_on "$sb" "$prompt"; then
      body=$(jq -n --arg owner "$task_owner" --arg lease "$lease" --arg result "done by $owner" \
        '{owner:$owner, lease_token:$lease, result:$result}')
      api -sf "$BASE/v1/cohorts/$(fid)/tasks/$tid/complete" -d "$body" >/dev/null
      echo "<< [$owner] done: $tid"
    else
      body=$(jq -n --arg owner "$task_owner" --arg lease "$lease" \
        '{owner:$owner, lease_token:$lease, error:"agent exec failed", requeue:true}')
      api -sf "$BASE/v1/cohorts/$(fid)/tasks/$tid/fail" -d "$body" >/dev/null
...
case "${1:-}" in
  up)
    prof="${2:-build-fleet}"
    id=$(api -sf "$BASE/v1/cohorts" -d "{\"profile\":\"$prof\"}" | jq -r '.id')
  ...
  add)
    ...
    api -sf "$BASE/v1/cohorts/$(fid)/tasks" -d "$(jq -n --arg p "$2" '{payload:$p}')" \
      | jq -r '"added task " + .id'
    ;;
  run)
    i=0
  ...
  exec)
    ...
    ;;
  status)
    api -sf "$BASE/v1/cohorts/$(fid)/tasks" \
  ...
  export)
    ...
    ;;
  down)
    api -sf -X DELETE "$BASE/v1/cohorts/$(fid)" >/dev/null && echo "fleet down"
    rm -f "$STATE"
    ;;
  *)
